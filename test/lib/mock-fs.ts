...
export function close(fd: number, callback: (err: NodeJS.ErrnoException) => void): void {
  ...
  if (fds[fd]) {
    ...
    return process.nextTick(
      callback,
      Object.assign(new Error('bad file descriptor: ' + fd), {
        errno: -9,
        code: 'EBADF',
        syscall: 'close',
      }),
...
export function init(newFakeFilesystem: FakeFilesystem): void {
  fakeFilesystem = JSON.parse(JSON.stringify(newFakeFilesystem))
  fds = []
}
...
export function mkdir(
  dirpath: string,
  options: number | string | fs.MakeDirectoryOptions | undefined | null | ((err?: NodeJS.ErrnoException) => void),
  callback?: (err?: NodeJS.ErrnoException) => void,
): void {
  ...
  if (recursive) {
    makeDir(dirpath, {mode, fs: mockFs as any})
      .then(() => {
...
export function readFileSync(filepath: string, _options: any): string {
